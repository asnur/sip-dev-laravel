...
const map = new mapboxgl.Map({
    ...
    center: [110.21786695260732, -7.4758890916220695],
    zoom: 13,
...
map.on("style.load", function () {
    ...
    map.on(clickEvent, function (e) {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: (e) => {
                const desa = e.features[0].properties.Desa;
                // console.log(kelurahan);
                var kelurahanStorage = localStorage.getItem("desa");
                if (kelurahanStorage !== desa) {
                    $("#btn-titik").show();
...
map.on("mouseenter", "ukm_fill", (e) => {
    ...
    <div class="card-body p-2">
      ...
      <div style="line-height: 1.2;">
        <span class="d-block"><b>Omzet :</b> ${dt["Omzet"]}</span>
        <span class="d-block"><b>Jenis :</b> ${dt["Jenis"]}</span>
        <span class="d-block"><b>Tenaga Kerja :</b> ${dt["Pegawai"]} Orang</span></div>
      </div>
    </div>`;
...
map.on(clickEvent, "wilayah_fill", function (e) {
    ...
    $(".inf-kota").html(dt.Desa);
    $(".inf-luasarea").html(dt["Luas Wilayah"] + " km2");
    $(".inf-kepadatan").html(separatorNum(dt["Kepadatan"]));
    $(".inf-rasio").html(separatorNum(dt["Jumlah Penduduk"]));
    $(".inf-05th").html(dt.U0);
    $(".inf-510th").html(dt.U5);
    $(".inf-1015th").html(dt.U10);
    $(".inf-1520th").html(dt.U15);
    $(".inf-2035th").html(dt.U20);
    $(".inf-3540th").html(dt.U35);
    $(".inf-4045th").html(dt.U40);
    $(".inf-4550th").html(dt.U45);
    $(".inf-5055th").html(dt.U50);
    $(".inf-5560th").html(dt.U55);
    $(".inf-6065th").html(dt.U60);
    $(".inf-6570th").html(dt.U65);
    $(".inf-7075th").html(dt.U70);
    $(".inf-75th").html(dt.U75);
    map.resize();
...
function getNJOP(e) {
    ...
    $.ajax({
        ...
        success: function (dt) {
            ...
            if (dtResp.features != null) {
                $(".inf-njop").html(
                    `Rp ${separatorNum(prop.NJOP)} per m&sup2;`
                );
            } else {
                $(".inf-njop").html(`Rp 0 per m&sup2;`);
            }
                    ...
                    success: function (res) {
                        if (res.features != null) {
                            ...
                            for (var i in dt) {
                                ...
                                resHTML += `<li class="wm-li-result wilayah-select" data-kordinat="${koor}" data-wilayah="${prop["Desa"]}"><i class="fa fa-map-marker" style="font-size: 15px;"></i> ${prop["Desa"]}</li>`;
                            }
                    ...
                    success: function (res) {
                        if (res.features != null) {
                            ...
                            for (var i in dt) {
                                ...
                                resHTML += `<li class="wm-li-result text_all wilayah-select" data-kordinat="${koor[1]},${koor[0]}" data-wilayah="${prop["Desa"]}"><i class="fa fa-map-marker" style="font-size: 15px;"></i> ${prop["Desa"]}</li>`;
                            }
